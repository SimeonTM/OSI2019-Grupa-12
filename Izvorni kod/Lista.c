...
#include "Korisnik_bez_naloga.h"
    void Read_Dj(DOGADJAJ *pom,CATEGORY src,LISTA *events)         //f-ja ucitava novi dogadjaj
    {
        char temp[200]="";
        int z;
        ...
        do
        {
            printf("\nUnesite godinu desavanja:");          scanf("%d",&i);
            printf("\nUnesite mjesec desavanja:");          scanf("%d",&j);
            printf("\nUnesite dan desavanja:");             scanf("%d",&k);
    ...
    int Delete_Dj(LISTA *lista,const char *key)
    {
        ...
        {
            if(lista->head ==q && lista->tail ==q)  lista->tail = lista->head = NULL;  //ako ima samo jedan element u listi

            else if(lista->head == q)  //ako se nalazi na pocetku liste
            {
                NODE *s=q->right;
                s->left=NULL;
                lista->head=s;
            ...
            else if(lista->tail == q)     //ako se nalazi na kraju liste
            {
                NODE *s=q->left;
                s->right= NULL;
                lista->tail=s;
            ...
            else                //ako se nalazi na sredini liste
            {
                NODE *s=q->left,*t=q->right;
                s->right=t;
                t->left=s;
            }
