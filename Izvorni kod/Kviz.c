...
void Read_Quiz(KVIZ *kviz)
{
    ...
    {
        fscanf(fp,"%s\n",pom);      strcpy(kviz->question[i],pom);
        fscanf(fp,"%s\n",pom);      strcpy(kviz->first_answer[i],pom);
        fscanf(fp,"%s\n",pom);      strcpy(kviz->second_answer[i],pom);
        fscanf(fp,"%s\n",pom);      strcpy(kviz->third_answer[i],pom);
        fscanf(fp,"%d\n",&kviz->correct[i]);
    }
    fclose(fp);
}
...
void Play_Quiz(KVIZ *kviz)
{
     ...
     {
         Remove_Underscores(kviz->question[i]       );     printf("\n %d. pitanje: %s\n",i+1,kviz->question[i]);
         Remove_Underscores(kviz->first_answer[i]   );     printf("    - 1. odgovor: %s\n",kviz->first_answer[i]);
         Remove_Underscores(kviz->second_answer[i]  );     printf("    - 2. odgovor: %s\n",kviz->second_answer[i]);
         Remove_Underscores(kviz->third_answer[i]   );     printf("    - 3. odgovor: %s\n",kviz->third_answer[i]);
...
void Add_Question( KVIZ *kviz,
                   const char *question,
                   const char *first_ans,
                   const char *second_ans,
                   const char *third_ans,
                   int correct )
{
